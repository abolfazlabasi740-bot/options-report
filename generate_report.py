...
def run_v4_pipeline(input_path, output_md="output_options_report.md", symbol_prefix=""):
    ...
    base_cols = [c for c in [
        "BaseSymbol","BaseMappingStatus","BaseInsCode","BaseSymbolResolved","BaseInstrumentName","BaseAsOf",
        "BaseLast","BaseClose","BasePrevClose","BasePriceChangePct","BaseVolume","BaseValue","BaseTradeCount",
        "BaseRealBuyVolume","BaseRealSellVolume","BaseLegalBuyVolume","BaseLegalSellVolume",
        "BaseRealBuyCount","BaseRealSellCount","BaseLegalBuyCount","BaseLegalSellCount",
        "BaseRealNetVolume","BaseLegalNetVolume","BaseRealBuyerAvgVolume","BaseRealSellerAvgVolume","BaseRealBuyerPower",
        "BaseReturn5D","BaseSMA5","BaseSMA20","BaseRSI14","BaseTrendPoints","BaseTrendBias","BaseHistoryRows",
        "Base_BestBidPrice","Base_BestBidVolume","Base_BestAskPrice","Base_BestAskVolume",
        "BaseMessageCount","BaseDataStatus","BaseDataError"
    ...
    generated = now_tehran(); scope = symbol_prefix or "کل بازار"

    def p_int(value):
        if pd.isna(value): return "داده موجود نیست"
        try: return f"{int(round(float(value))):,}".replace(",","٬").translate(str.maketrans("0123456789","۰۱۲۳۴۵۶۷۸۹"))
        except (TypeError, ValueError, OverflowError): return "داده موجود نیست"
    def p_decimal(value, digits=2, signed=False, trim=False):
        if pd.isna(value): return "داده موجود نیست"
        try: n=float(value)
        except (TypeError, ValueError, OverflowError): return "داده موجود نیست"
        sign="+" if signed and n>0 else "" if n>=0 else "−"; n=abs(n); text=f"{n:,.{digits}f}"
        if trim: text=text.rstrip("0").rstrip(".")
        return sign+text.replace(",","٬").replace(".","٫").translate(str.maketrans("0123456789","۰۱۲۳۴۵۶۷۸۹"))
    def p_text(value):
        if pd.isna(value) or str(value).strip() in {"","nan","None"}: return "داده موجود نیست"
        return str(value).strip()

    lines=["📊 گزارش رتبه‌بندی اختیار معامله — V4.1",f"پروتکل: {V4_PROTOCOL}",f"حداقل اهرم: {p_decimal(MIN_LEVERAGE,1,trim=True)}",f"دامنه: {scope}",f"تعداد قراردادهای گزارش‌شده: {p_int(len(top))}",""]
    for _,r in top.iterrows():
        expiry=p_text(r.get("تاریخ سررسید",r.get("سررسید",np.nan))); remaining=p_int(r.get("RemainingDays",np.nan))
        lines.extend(["━━━━━━━━━━━━━━━━",f"🔹 {int(r['رتبه'])}. {p_text(r.get('نماد',np.nan))}","",f"قیمت اعمال: {p_int(r.get('قیمت اعمال',np.nan))} | آخرین: {p_int(r.get('آخرین قیمت',np.nan))}",f"سر‌به‌سر: {p_int(r.get('سر به سر',np.nan))} | پایه: {p_int(r.get('قیمت سهم پایه',np.nan))}",f"اهرم: {p_decimal(r.get('اهرم',np.nan),2,trim=True)} | فاصله سر‌به‌سر: {p_decimal(r.get('BreakevenDistancePct',np.nan),3,signed=True)}%",f"سررسید: {expiry} ({remaining} روز)",f"امتیاز: {p_decimal(r.get('FinalScore',np.nan),2)}",""])
    lines.extend(["━━━━━━━━━━━━━━━━","📌 کنترل گزارش",f"ردیف اولیه: {p_int(total_initial)}",f"پس از Scope: {p_int(guard['input_rows'])}",f"پس از V4 Guard: {p_int(guard['output_rows'])}",f"حذف‌شده توسط Guard: {p_int(guard['dropped_rows'])}",f"زمان تولید: {generated.strftime('%Y-%m-%d %H:%M:%S %Z')}",f"فایل مبنا: {metadata['filename']}",f"SHA-256: {metadata['sha256']}","","📌 توضیح: امتیاز، رتبه‌بندی کیفیت قرارداد است و سیگنال قطعی خرید/فروش نیست."])
    if base_audit:
        lines.extend(["","📊 کانتکست سهم پایه",f"وضعیت داده: {p_text(base_audit.get('status',np.nan))}",f"نمادهای پایه: {p_int(base_audit.get('symbols',np.nan))}",f"پاسخ معتبر: {p_int(base_audit.get('resolved',np.nan))}",f"بدون پاسخ معتبر: {p_int(base_audit.get('failed',np.nan))}"])
    report="\n".join(lines)+"\n"; Path(output_md).write_text(report,encoding="utf-8"); top.to_csv(OUTPUT_TOP,index=False,encoding="utf-8-sig")
    print(f"[OK] V4.1 report: {output_md} | rows={len(top)}")
    return report

if __name__ == "__main__":
    explicit=sys.argv[1] if len(sys.argv)>1 else None; prefix=os.getenv("REPORT_SYMBOL_PREFIX","").strip(); input_path,metadata=find_input(explicit); print(f"Canonical input: {input_path.name}"); print(run_v4_pipeline(input_path,symbol_prefix=prefix))
