...
def main() -> None:
    ...
    tsetmc_mode = audit.get("source_of_truth") == "TSETMC" and bool(audit.get("data_mode"))
    ...
    if tsetmc_mode:
        ...
        if audit.get("live_movement_claim") != "NOT_CLAIMED":
            raise RuntimeError("گزارش TSETMC نباید حرکت زنده اثبات‌نشده را ادعا کند")
        if not audit.get("snapshot_sha256"):
            raise RuntimeError("هش Snapshot TSETMC موجود نیست")
    else:
